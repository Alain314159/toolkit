...
function extraerIdentificadores(exprSrc, locales) {
  ...
  // Envolver en funcion para manejar tanto expresiones como statements
  let ast;
  const envuelto = '(function __tk__() { ' + exprSrc + ' });';
  try {
    ast = acorn.parse(envuelto, { ecmaVersion: 'latest', sourceType: 'module' });
  } catch (e) {
    // Fallback: intentar como expresion pura
    try {
...
export class VueFile {
  ...
  _tieneComaFinal(nodo) {
    const idx = this.scriptOffset + nodo.end;
  ...
  _removeScriptItem(seccion, nombre) {
    let items;
    if (seccion === 'methods') items = this.getMethods();
    else if (seccion === 'computed') items = this.getComputed();
    else if (seccion === 'data') items = this.getDataFields();
    else return false;

    const item = items.find(x => x.nombre === nombre);
    if (!item) return false;

    const start = this.scriptOffset + item.nodo.start;
    let end = this.scriptOffset + item.nodo.end;
    if (this._tieneComaFinal(item.nodo)) end++;
    // Extender hasta el fin de linea
    while (end < this.content.length) {
      const ch = this.content[end];
      if (ch === '\n') { end++; break; }
      if (ch === ' ' || ch === '\t' || ch === '\r') { end++; continue; }
      break;
    }

    this.content = this.content.slice(0, start) + this.content.slice(end);
    this.dirty = true;
    this.operations.push(seccion + '.' + nombre);
    this._parse();
    return true;
  }

  removeMethod(nombre) {
    return this._removeScriptItem('methods', nombre);
  }

  removeComputed(nombre) {
    return this._removeScriptItem('computed', nombre);
  }

  removeDataField(nombre) {
    return this._removeScriptItem('data', nombre);
  }

  removeTemplateSection(openComment, closeComment) {
    const idxIni = this.content.indexOf(openComment);
    if (idxIni < 0) return false;
    const idxFin = this.content.indexOf(closeComment, idxIni + openComment.length);
    if (idxFin < 0) return false;
    this.content = this.content.slice(0, idxIni) + this.content.slice(idxFin);
    this.dirty = true;
    this.operations.push('template:' + openComment.slice(0, 50));
    this._parse();
    return true;
  }

  removeScriptSection(inicioComentario, finComentario) {
    const idxIni = this.content.indexOf(inicioComentario);
    if (idxIni < 0) return false;
    const idxFin = this.content.indexOf(finComentario, idxIni + inicioComentario.length);
    if (idxFin < 0) return false;
    this.content = this.content.slice(0, idxIni) + this.content.slice(idxFin);
    this.dirty = true;
    this.operations.push('script:' + inicioComentario.slice(0, 50));
    this._parse();
    return true;
  }

  removeImport(name) {
    if (!this.scriptAST) return false;
    const imports = this.scriptAST.body.filter(n => n.type === 'ImportDeclaration');
    for (const imp of imports) {
      const specifiers = imp.specifiers;
      const idxSpec = specifiers.findIndex(sp => sp.local.name === name);
      if (idxSpec < 0) continue;

      const start = this.scriptOffset + imp.start;
      const end = this.scriptOffset + imp.end;

      if (specifiers.length === 1) {
        let fin = end;
        while (fin < this.content.length && (this.content[fin] === '\n' || this.content[fin] === ' ')) fin++;
        this.content = this.content.slice(0, start) + this.content.slice(fin);
      } else {
        const spec = specifiers[idxSpec];
        const specStart = this.scriptOffset + spec.start;
        const specEnd = this.scriptOffset + spec.end;
        let s2 = specStart;
        let e2 = specEnd;
        let k = s2 - 1;
        while (k > 0 && (this.content[k] === ' ' || this.content[k] === '\t')) k--;
        if (this.content[k] === ',') s2 = k;
        else {
          k = e2;
          while (k < this.content.length && (this.content[k] === ' ' || this.content[k] === '\t')) k++;
          if (this.content[k] === ',') e2 = k + 1;
        }
        this.content = this.content.slice(0, s2) + this.content.slice(e2);
      }
      this.dirty = true;
      this.operations.push('import.' + name);
      this._parse();
      return true;
    }
    return false;
