...
export async function aplicarPlanTransaccional(planes, opciones = {}) {
  const { backup = true, silent = false, dryRun = false } = opciones;
  const reporte = {
    ok: false,
    total: planes.length,
    preparados: [],
  ...
  // Si dryRun, terminar aqui sin escribir
  if (dryRun) {
    reporte.ok = true;
    reporte.dryRun = true;
    reporte.commitCount = reporte.preparados.filter(p => !p.skipped).length;
